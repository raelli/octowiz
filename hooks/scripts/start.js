...
async function ensureA2AServer({
  ...
  if (await isPortOpen(port)) {
    ...
    if (!expectedVersion)
      return // nothing to compare against — leave the server alone
    ...
    if (state !== 'stale')
      return // fresh, or unreachable mid-probe

    // Stale octowiz server: only ever kill the pid we recorded at spawn time.
    let pid = Number.NaN
    try {
      pid = Number.parseInt(fs.readFileSync(path.join(config.cacheDir(), 'a2a-agent.pid'), 'utf8').trim(), 10)
    }
    catch {}
